...
class ViewController: UIViewController, WKNavigationDelegate {
    ...
    @objc func keyboardWillHide(_ notification: NSNotification) {
        LovLov.webView.setNeedsLayout()
    }
    
    func initWebView() {
        LovLov.webView = createWebView(container: webviewView, WKSMH: self, WKND: self, NSO: self, VC: self)
        webviewView.addSubview(LovLov.webView);
        LovLov.webView.uiDelegate = self;
        LovLov.webView.addObserver(self, forKeyPath: #keyPath(WKWebView.estimatedProgress), options: .new, context: nil)
        
        if #available(iOS 15.0, *), adaptiveUIStyle {
            themeObservation = LovLov.webView.observe(\.underPageBackgroundColor) { [unowned self] webView, _ in
                currentWebViewTheme = LovLov.webView.underPageBackgroundColor.isLight() ?? true ? .light : .dark
...
//        self.splashBkgView.frame = CGRect(x: splashBkgFrame.minX, y: statusBarHeight, width: splashBkgFrame.width, height: splashBkgFrame.height)
    ...
    func overrideUIStyle(toDefault: Bool = false) {
        if #available(iOS 15.0, *), adaptiveUIStyle {
            if (((htmlIsLoaded && !LovLov.webView.isHidden) || toDefault) && self.currentWebViewTheme != .unspecified) {
                UIApplication
                    .shared
                    .connectedScenes
    ...
    @objc func loadRootUrl() {
        ...
        LovLov.webView.load(URLRequest(url: launchUrl))
    }
    ...
    override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        ...
        if (keyPath == #keyPath(WKWebView.estimatedProgress) &&
                LovLov.webView.isLoading &&
                !self.loadingView.isHidden &&
                !self.htmlIsLoaded) {
                    var progress = Float(LovLov.webView.estimatedProgress);
    ...
    deinit {
        LovLov.webView.removeObserver(self, forKeyPath: #keyPath(WKWebView.estimatedProgress))
    }
}
...
extension ViewController: WKScriptMessageHandler {
  func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
        if message.name == "print" {
            printView(webView: LovLov.webView)
        }
